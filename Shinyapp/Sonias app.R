...
#Creating Leaflet for pesticide use 
        ...
        tabPanel("Comparison of California Low Birth Weight and Pesticide use by County", icon = icon("chart-bar"),
                 sidebarPanel( 
                     p("We compared uses of pesticide and low birth weight/preterm birth outcomes by county in California. The data spans from 2007 to 2016 
                       and we have only included counties that have a county population of >100,000. The graph on the top
                       shows a bargraph of % in low birth weight/preterm birth by county from 2007-2016. The bargraph on the bottom compares
                       pesticide use (in pounds) by county from the same span of years."),
                     br(),
                     ...
                     p("Can you guess which counties use the highest amounts of pesticide across the years?")
                 ), #closing sidebarpanel 
                mainPanel(
                    plotOutput("bargraph"),
        ...
        tabPanel("Comparison of California Pesticide use by County", icon = icon("map-pin"),
                 sidebarPanel( 
                     p("Finally, we created a map that tracks the uses of pesticide in the county of California. As we can see, pesticide use seems to be pronounced in the central part of the state. This is also known as San Joaquin valley and 
                       includes the entire counties of Fresno, San Joaquin, Tulare, Stanislaus, Merced, Madera, Kings and part of Kern County.
                       The valley is also known as the most productive agricultural region in the world."),
                     br()
                 ), #closing sidebarpanel 
                 mainPanel(
                     leafletOutput("leaflet_pesticide")
                 )#closing mainpanel
        )#closing tabpanel
    )#closing tabsetpanel
)#closing fluidpage
